...
import { fetchProtocols, fetchMedicationBase, createProtocol, updateProtocol, deleteProtocol as deleteProtocolApi } from '../services/dataService';
...
const MedicationList: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const [protocolsData, medsData] = await Promise.all([
        fetchProtocols(),
        fetchMedicationBase()
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        const updated = await updateProtocol(editingId, updates);
