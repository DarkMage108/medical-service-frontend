...
const PermissionsManager: React.FC = () => {
  ...
  const handleSaveRole = async (role: string) => {
    try {
      ...
      setSuccessMessage(`Permissões do perfil ${ROLE_LABELS[role as UserRole] || role} atualizadas com sucesso!`);
  ...
  const handleResetRole = async (role: string) => {
    if (!window.confirm(`Deseja restaurar as permissões padrão para o perfil ${ROLE_LABELS[role as UserRole] || role}?`)) {
      return;
    }
    ...
    try {
      ...
      setSuccessMessage(`Permissões do perfil ${ROLE_LABELS[role as UserRole] || role} restauradas para o padrão!`);
      setHasChanges(prev => ({ ...prev, [role]: false }));
    } catch (err: any) {
      setError(err.message || 'Erro ao restaurar permissões');
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-3 text-slate-600">Carregando permissões...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-slate-500 mt-1">
          Configure quais menus cada perfil de usuário pode acessar no sistema.
        </p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="text-sm font-bold text-blue-800 mb-2">Informações</h3>
        <ul className="text-sm text-blue-700 space-y-1">
          <li>As alterações de permissão entram em vigor imediatamente após salvar.</li>
          <li>Os usuários precisarão recarregar a página para ver as alterações.</li>
          <li>O menu Dashboard normalmente deve permanecer acessível para todos os perfis.</li>
        </ul>
      </div>
    </div>
  );
